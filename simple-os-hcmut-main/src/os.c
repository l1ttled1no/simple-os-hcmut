...
static void read_config(const char *path)
{
	...
	printf("Time_slot: %d, CPU %d, num_processes: %d", time_slot, num_cpus, num_processes);
...
// #endif
// #endif
// 	printf("After MM_PAGING block\n");

#ifdef MM_PAGING
    printf("MM_PAGING is defined\n");
    int sit;
    #ifdef MM_FIXED_MEMSZ
        printf("MM_FIXED_MEMSZ is defined\n");
        ...
        for (sit = 1; sit < PAGING_MAX_MMSWP; sit++)
            memswpsz[sit] = 0;
        printf("Set fixed memory sizes: memramsz = 0x%x, memswpsz[0] = 0x%x\n", memramsz, memswpsz[0]);
        
        #ifdef MM_PAGING_HEAP_GODOWN
            printf("MM_PAGING_HEAP_GODOWN is defined\n");
            vmemsz = 0x300000;
            printf("Set vmemsz = 0x%x\n", vmemsz);
        #else
            printf("MM_PAGING_HEAP_GODOWN is not defined\n");
        #endif
    #else
        printf("MM_FIXED_MEMSZ is not defined\n");
        /* Read input config of memory size: MEMRAM and up to 4 MEMSWP (mem swap)
         * Format: (size=0 result non-used memswap, must have RAM and at least 1 SWAP)
         *        MEM_RAM_SZ MEM_SWP0_SZ MEM_SWP1_SZ MEM_SWP2_SZ MEM_SWP3_SZ
         */
        int read_items = fscanf(file, "%d\n", &memramsz);
        printf("Read memramsz = %d (items read: %d)\n", memramsz, read_items);
        
        for (sit = 0; sit < PAGING_MAX_MMSWP; sit++) {
            read_items = fscanf(file, "%d", &(memswpsz[sit]));
            printf("Read memswpsz[%d] = %d (items read: %d)\n", sit, memswpsz[sit], read_items);
        }
        
        #ifdef MM_PAGING_HEAP_GODOWN
            printf("MM_PAGING_HEAP_GODOWN is defined\n");
            read_items = fscanf(file, "%d\n", &vmemsz);
            printf("Read vmemsz = %d (items read: %d)\n", vmemsz, read_items);
        #else
            printf("MM_PAGING_HEAP_GODOWN is not defined\n");
        #endif
        
        fscanf(file, "\n"); /* Final character */
        printf("Read final newline character\n");
    #endif
#else
    printf("MM_PAGING is not defined\n");
#endif
